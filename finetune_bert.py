...
parser.add_argument('--dataset', default='sms', choices=['sms', 'sms10', 'sms20', 'fbs', 'fbs10','fbs20'])
parser.add_argument('--bert_init', type=str, default='hfl/chinese-bert-wwm-ext',
                    choices=['hfl/chinese-bert-wwm-ext', 'hfl/chinese-debert-wwm-ext', 'hfl/chinese-ernie-base',
                             ...
                             'hfl/chinese-debert-wwm-ext','hfl/chinese-macbert-base','hfl/roc-bert-base-zh','hfl/ShannonAIChineseBERT-base2'])
...
optimizer = th.optim.Adam(model.parameters(), lr=bert_lr,weight_decay=0.1)#,weight_decay=0.1
...
test_loader = Data.DataLoader(datasets['test'], batch_size=len(datasets['test']), shuffle=False)
...
# 打印测试结果
logger.info("测试准确率: {:.4f} 损失: {:.4f}".format(test_acc, test_nll))
logger.info("混淆矩阵:\n{}".format(conf_matrix))
logger.info("分类报告:\n{}".format(class_report))